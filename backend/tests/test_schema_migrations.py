...
def test_applied_version_is_recorded(test_conn):
    """Nach der Initialisierung ist jede vorhandene Migration verzeichnet -
    verglichen wird gegen die Dateien auf der Platte, nicht gegen eine feste
    Nummer (die müsste sonst bei jeder neuen Migration nachgezogen werden)."""
    expected = [(m.version, m.name) for m in migrations_module.discover_migrations()]
    assert migrations_module.current_version(test_conn) == expected[-1][0]
...
def test_a_new_migration_is_applied_and_recorded(test_conn, tmp_path):
    """Die Probenversionen liegen bewusst bei 9xx: eine Nummer, die eine echte
    Migration bereits belegt, gälte als angewendet und würde übersprungen -
    der Test prüfte dann stillschweigend nichts mehr."""
    (tmp_path / "001_initial_postgres.sql").write_text(
        "-- bereits angewendet, Inhalt hier irrelevant\nSELECT 1;", encoding="utf-8"
    )
    (tmp_path / "900_add_probe_table.sql").write_text(
    ...
    assert applied == ["900"]
    assert migrations_module.current_version(test_conn) == "900"
...
def test_failing_migration_aborts_and_rolls_back_completely(test_conn, tmp_path):
    ...
    assert migrations_module.current_version(test_conn) == latest_before
...
def test_migrations_never_delete_existing_data(test_conn, tmp_path):
    ...
    (tmp_path / "902_noop.sql").write_text(
